...
try {
    ...
    // Try to get Facebook login route, use direct URL if it fails
    try {
        $facebookLoginRoute = route('login.facebook');
    } catch (\Exception $e) {
        $facebookLoginRoute = '/login/facebook';
    }
} catch (\Exception $e) {
    $homeRoute = '/';
    $loginRoute = '/login';
        ...
        <div class="flex-1 flex items-center justify-center bg-custom-bg p-4 lg:p-8 lg:max-w-[600px] lg:mr-8">
            <div class="w-full max-w-md space-y-8 px-4 sm:px-8 py-8 sm:py-10 bg-white rounded-3xl shadow-xl">
                ...
                <div class="flex space-x-4">
                    ...
                    <a href="{{ $facebookLoginRoute }}" class="flex-1 flex items-center justify-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-xs sm:text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
